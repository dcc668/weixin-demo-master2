...
public class JedisClient {
    ...
    private void init() {
        ...
        jedisPool = new JedisPool(config, resourceBundle.getString("redis.host"), new Integer(resourceBundle.getString("redis.port")), new Integer(resourceBundle.getString("redis.timeout")), resourceBundle.getString("redis.password"));
    ...
    public String getMapField(String key, String field) {
        ...
        try {
            ...
            logger.warn("getMapField {}.{} = {}", key, field, value, e);
        } finally {
            returnResource(jedis);
        }
        return value;
        ...
        try {
            ...
            if (jedis.exists(getBytesKey(key))) {
                ...
                for (Map.Entry<byte[], byte[]> e : map.entrySet()) {
                    value.put(TStringUtils.toString(e.getKey()),
                            toObject(e.getValue()));
                }
    ...
    public byte[] getBytesKey(Object object) {
        if (object instanceof String) {
            return TStringUtils.getBytes((String) object);
        } else {
            return TObjectUtils.serialize(object);
        }
    }
    ...
    public Object toObject(byte[] bytes) {
        return TObjectUtils.unserialize(bytes);
    }
     ...
     * @return
     */
    public long setHincrBy(String key, String field, long value) {
        long result = 0;
        Jedis jedis = null;
        try {
            jedis = getResource();
    ...
    public Long hIncrBy(String key, String field, long val) {
        ...
        try {
            jedis = getResource();
            if (jedis.exists(key)) {
                rst = jedis.hincrBy(key, field, val);
                logger.debug("hIncrBy {} : {} : {}", key, field, val);
            }
        } catch (Exception e) {
            logger.warn("hIncrBy {} : {} : {}", key, field, val, e);
        } finally {
            returnResource(jedis);
        }
        return rst;
    ...
    public long hSet(String key, String field, String val) {
        ...
        try {
            jedis = getResource();
            rst = jedis.hset(key, field, val);
            logger.debug("hSet {} : {} : {}", key, field, val);
        } catch (Exception e) {
            logger.warn("hSet {} : {} : {} : {}", key, field, val, e);
        } finally {
            returnResource(jedis);
        }
        return rst;
